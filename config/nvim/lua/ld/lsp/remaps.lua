...
function M.set_default(client, bufnr)
  ...
  if cap.definitionProvider then
    ...
    buf_set_keymap('n','<leader>lw', "<cmd>lua require('telescope.builtin').lsp_workspace_symbols()<CR>", 'lsp', 'lsp_workspace_symbols', 'Workspace symbols')
  end
  -- if cap.declarationProvider then
  -- map('n', 'gd', '<cmd>lua vim.lsp.buf.declaration()<CR>', opts)
  -- end
  if cap.implementationProvider then
    buf_set_keymap('n','<leader>li', '<cmd>lua vim.lsp.buf.implementation()<CR>', 'lsp', 'lsp_goto_implementation', 'Go to implementation')
  end
  if cap.referencesProvider then
    -- buf_set_keymap('n','<leader>tr', '<cmd>lua vim.lsp.buf.references()<CR>', opts)
    buf_set_keymap('n','<leader>lu', "<cmd>lua require('telescope.builtin').lsp_references()<CR>", 'lsp', 'lsp_references', 'Show references')
  end

  -- buf_set_keymap('n','<leader>th', '<cmd>lua vim.lsp.buf.hover()<CR>', opts)
  buf_set_keymap('n', '<leader>lh', "<cmd>lua require('lspsaga.hover').render_hover_doc()<CR>", 'lsp', 'lsp_hover_docs', 'Hover documentation')

  if cap.documentSymbolProvider then
    -- buf_set_keymap('n','<leader>to', '<cmd>lua vim.lsp.buf.document_symbol()<CR>', opts)
    buf_set_keymap('n','<leader>ls', "<cmd>lua require('telescope.builtin').lsp_document_symbols()<CR>", 'lsp', 'lsp_document_symbols', 'Document symbols')
  end

  buf_set_keymap('n', '<leader>lv', "<cmd>lua require('lspsaga.signaturehelp').signature_help()<CR>", 'lsp', 'lsp_signature_help', 'Show signature')
  ...
  if cap.codeActionProvider then
    ...
    buf_set_keymap('n','<leader>la', "<cmd>lua require('telescope.builtin').lsp_code_actions({ timeout = 1000 })<CR>", 'lsp', 'lsp_code_actions', 'Code actions')
    buf_set_keymap('v', '<leader>la', "<cmd>lua require('telescope.builtin').lsp_range_code_actions({ timeout = 1000 })<CR>", 'lsp', 'lsp_code_actions_in_visual', 'Code actions (visual)')
  ...
  -- buf_set_keymap('n','<leader>fe', '<cmd>:LspDiagnostics 0<CR>', opts)
   buf_set_keymap('n','<leader>fe', "<cmd>lua require('ld.lsp.functions').show_diagnostics()<CR>", 'lsp', 'lsp_show_diagnostics', 'Show diagnostics')
