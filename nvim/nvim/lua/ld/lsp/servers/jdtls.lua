...
local workspace_folder = home .. "/.local/share/java/eclipse/" .. vim.fn.fnamemodify(root_dir, ":p:h:t")
local java_bin = function()
	if utils.is_windows() then
		return os.getenv("JAVA_HOME") .. "/bin/java"
	else
		return home .. "/.local/share/java/17/bin/java"
	end
end
local configuration_path = function()
	if utils.is_windows() then
		return home .. "/.local/share/java/jdtls/config_win"
	else
...
config.cmd = {
	...
	"-jar",
	vim.fn.glob(home .. "/.local/share/java/jdtls/plugins/org.eclipse.equinox.launcher_*.jar"),
	"-configuration",
	configuration_path(),
	"-data",
	workspace_folder,
